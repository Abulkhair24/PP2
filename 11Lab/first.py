...
def function_for_adding_user(name, surname, tel):
    ...
    if len(data) == 0:
        ...
        cur.execute(f"UPDATE some_data SET name = '{name}', surname = '{surname}', tel = '{tel}' WHERE name = '{name}' and surname = '{surname}'")
        print("Successfully updated data of database.")
    connect.commit()

list_of_function = ["1 - ENTER A NEW USER", "2 - CHANGE DATA OF USER", "4 - DELETE USER",
 "5 - INSERT DATA By CSV FILE", "6 - SORTING BY NAME", "7 - SORTING BY SURNAME", "8 - SORTING BY TELEPHONE", 
 "W - SEARCH DATA BY PATTERN NAME", "Q - SEARCH DATA BY PATTERN SURNAME", "E - SEARCH DATA BY PATTERN TELEPHONE", 
 "R - ENTERING INTO DATA SEVERAL USERS", "T - GET A DATA OF SEVERAL USERS", "0 - TURN OF THE DATABASE"]
